            ...
            <div class="card-body">
                ...
                <form class="user" action="" id="form" name="form_edit" enctype="multipart/form-data" method="post" accept-charset="utf-8">
                    ...
                    <fieldset id="vendas" class="mt-4 border p-2">
                        <h5>Venda Nº <?= $venda->venda_id ?>
                            <?php if ($venda->venda_status == 'ABERTA') : ?>
                                <span class="badge badge-primary"><?= $venda->venda_status ?></span>
                            <?php else : ?>
                                <span class="badge badge-success"><?= $venda->venda_status ?></span>
                            <?php endif; ?>
                        </h5>
                            ...
                            <table id="table_produtos" class="table">
                                ...
                                <tbody id="lista_produtos" class="item">
                                    ...
                                    <?php foreach ($venda_produtos as $venda_produto) : ?>
                                        ...
                                        <tr>
                                            <td><input disabled type="hidden" name="produto_id[]" value="<?php echo $venda_produto->venda_produto_id_produto; ?>" data-cell="A<?php echo $i; ?>" data-format="0" readonly></td>
                                            <td><input disabled title="Descrição do produto" type="text" name="produto_descricao[]" value="<?php echo $venda_produto->produto_descricao; ?>" class="produto_descricao form-control form-control-user input-sm" data-cell="B<?php echo $i; ?>" readonly></td>
                                            <td><input disabled title="Valor unitário do produto" name="produto_preco_venda[]" value="<?php echo $venda_produto->venda_produto_valor_unitario; ?>" class="form-control form-control-user input-sm text-right money pr-1" data-cell="C<?php echo $i; ?>" data-format="R$ 0,0.00" readonly></td>
                                            <td><input disabled title="Digite a quantidade apenas em número inteiros" type="text" inputmode="numeric" pattern="[-+]?[0-9]*[.,]?[0-9]+" name="produto_quantidade[]" value="<?php echo $venda_produto->venda_produto_quantidade; ?>" class="qty form-control form-control-user text-center" data-cell="D<?php echo $i; ?>" data-format="0[.]00" required></td>
                                            <td><input disabled title="Insira o desconto" name="produto_desconto[]" class="form-control form-control-user input-sm text-right" value="<?php echo $venda_produto->venda_produto_desconto; ?>" data-cell="E<?php echo $i; ?>" data-format="0,0[.]00 %" required></td>
                                            <td><input disabled title="Valor total do produto selecionado" name="produto_item_total[]" value="<?php echo $venda_produto->venda_produto_valor_total; ?>" class="form-control form-control-user input-sm text-right pr-1" data-cell="F<?php echo $i; ?>" data-format="R$ 0,0.00" data-formula="D<?php echo $i; ?>*(C<?php echo $i; ?>-(C<?php echo $i; ?>*E<?php echo $i; ?>))" readonly></td>
                                        </tr>
                                ...
                                <tfoot>
                                    <tr class="">
                                        ...
                                        <td class="text-right border-0">
                                            <input type="text" name="venda_valor_desconto" class="form-control form-control-user text-right pr-1" data-format="$ 0,0.00" data-cell="L1" data-formula="SUM(H1:H5)" readonly="" disabled>
                                        </td>
                                    ...
                                    <tr class="">
                                        ...
                                        <td class="text-right border-0">
                                            <input type="text" name="venda_valor_total" class="form-control form-control-user text-right pr-1" data-format="$ 0,0.00" data-cell="G2" data-formula="SUM(F1:F5)" readonly="" disabled>
                                        </td>
                                        <td class="border-0">&nbsp;</td>
                                    </tr>
                                </tfoot>
                    ...
                    <fieldset class="mt-4 border p-2">
                        ...
                        <div class="">
                            ...
                            <div class="form-group row">
                                <div class="col-md-6">
                                    <label class="small my-0">Selecione um Termo <span class="text-danger">*</span></label>
                                    <select id="id_termo" class="custom-select termo" name="termo" disabled>
                                        <!-- <option value="">Escolha</option> -->
                                        <?php foreach ($termos as $termo) : ?>
                                            <option value="<?= $termo->id; ?>" <?= ($termo->id == $venda->venda_termos) ? 'selected' : '' ?>><?= $termo->titulo; ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                            </div>
                    ...
                    <div class="mt-3">
                        <?php if ($venda->venda_status == 'ABERTA') : ?>
                            <a href="<?= site_url('vendas/edit/' . $venda->venda_id) ?>" class="btn btn-primary btn-sm mr-2"> <i class="fas fa-pen"></i>&nbsp;Editar</a>
                            <a href="<?= site_url('vendas/faturar/' . $venda->venda_id) ?>" class="btn btn-success btn-sm mr-2"> <i class="fas fa-check"></i>&nbsp;Faturar</a>
                        <?php endif; ?>
                        
                        <a href="<?php echo base_url('vendas'); ?>" class="btn btn-secondary btn-sm"><i class="fas fa-arrow-left"></i>&nbsp;Voltar</a>
                    </div>
